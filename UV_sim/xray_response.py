...
#    pkl_file_x = open('spectra_bin/'+abund+'/spectra_xray'+'_'+str(t[i])+'_'+str(np.log10(density[i]))+'.pkl', 'rb')
    xname = "spectra_xray_{:.2}_{}.pkl".format(t[i],np.log10(density[i]))
    xname = os.path.join(abund,xname)
    xname = os.path.join("spectra_bin",xname)
    pkl_file_x = open(xname, 'rb')
...
#    pkl_file = open('spectra_bin/'+abund+'/spectra_XMM_'+str(t[i])+'_'+str(np.log10(density[i]))+'nh'+str(NH)+'.pkl', 'rb')
    xname = "spectra_XMM_{:.2}_{}nh{}.pkl".format(t[i],np.log10(density[i]),NH)
    xname = os.path.join(abund,xname)
    xname = os.path.join("spectra_bin",xname)
    pkl_file = open(xname, 'rb')
...
#    xname = 'spectra_bin/'+abund+'/spectra_'+tag+'_'+str(t[i])+'_'+str(np.log10(density[i]))+'.pkl'
    xname = "spectra_{}_{:.2}_{}.pkl".format(tag,t[i],np.log10(density[i]))
    xname = os.path.join(abund,xname)
    xname = os.path.join("spectra_bin",xname)
    pkl_file_x = open(xname, 'rb')
...
#    fname = dir_name+'spectra_'+str(tag)+'_'+ str(t[i])+'_'+str(np.log10(density[i]))+'.pkl'
    fname = "spectra_{}_{:.2}_{}.pkl".format(tag,t[i],np.log10(density[i]))
...
#      s = (ch.spectrum(temperatures, density[i], wvl, filter = (chfilters.gaussian,.1), em=emeasures,verbose=verbose))
      s = (ch.mspectrum(temperatures, density[i], wvl, filter = (chfilters.gaussian,.1), em=emeasures, proc=p,verbose=verbose, doContinuum=1, minAbund=1.e-100))
...
# bin the number of photons per energy channel present in the input spectrum
  ...
  for i in range(0,len(spectrum[0])):
    ...
    if ((e_kev > min_en) and (e_kev < max_en) and (e_kev > cut[0]) and (e_kev < cut[1])):
      print(np.shape(spectrum[1]))
      print(np.shape(spectrum[0]))
